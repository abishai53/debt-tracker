import express, { type Request, Response, NextFunction } from "express";
import { registerRoutes } from "./routes";
import { setupVite, serveStatic, log } from "./vite";

/**
 * Initialize Express server with middleware and routes
 */
async function initializeServer() {
  const app = express();
  
  // Basic middleware
  app.use(express.json());
  app.use(express.urlencoded({ extended: false }));
  
  // Add API request logging middleware
  addRequestLoggingMiddleware(app);
  
  // Register routes and configure auth
  const server = await registerRoutes(app);
  
  // Add error handling middleware
  addErrorHandlingMiddleware(app);
  
  // Set up environment-specific configurations
  if (app.get("env") === "development") {
    await setupVite(app, server);
  } else {
    serveStatic(app);
  }
  
  // Start server on port 50000
  startServer(server);
}

/**
 * Add request logging middleware for API endpoints
 */
function addRequestLoggingMiddleware(app: express.Express) {
  app.use((req, res, next) => {
    const start = Date.now();
    const path = req.path;
    let capturedJsonResponse: Record<string, any> | undefined = undefined;
    
    // Intercept JSON responses to log them
    const originalResJson = res.json;
    res.json = function (bodyJson, ...args) {
      capturedJsonResponse = bodyJson;
      return originalResJson.apply(res, [bodyJson, ...args]);
    };
    
    // Log API requests when they complete
    res.on("finish", () => {
      const duration = Date.now() - start;
      
      if (path.startsWith("/api")) {
        let logLine = `${req.method} ${path} ${res.statusCode} in ${duration}ms`;
        
        if (capturedJsonResponse) {
          logLine += ` :: ${JSON.stringify(capturedJsonResponse)}`;
        }
        
        // Truncate long log lines
        if (logLine.length > 80) {
          logLine = logLine.slice(0, 79) + "…";
        }
        
        log(logLine);
      }
    });
    
    next();
  });
}

/**
 * Add global error handling middleware
 */
function addErrorHandlingMiddleware(app: express.Express) {
  app.use((err: any, _req: Request, res: Response, _next: NextFunction) => {
    const status = err.status || err.statusCode || 500;
    const message = err.message || "Internal Server Error";
    
    res.status(status).json({ message });
    throw err;
  });
}

/**
 * Start the Express server
 */
function startServer(server: any) {
  const port = 50000;
  
  server.listen({
    port,
    host: "0.0.0.0",
    reusePort: true,
  }, () => {
    log(`Server running on port ${port}`);
  });
}

// Initialize and start the server
initializeServer().catch(err => {
  console.error("Failed to start server:", err);
  process.exit(1);
});
